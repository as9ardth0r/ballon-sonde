...
#define REG_CALIB_00  0x88U /* dig_T1..dig_P9, 26 octets */
#define REG_CALIB_H1  0xA1U
#define REG_CALIB_26  0xE1U /* dig_H2..dig_H6, 7 octets */
...
static bool read_reg(uint8_t reg, uint8_t *data, uint32_t len) {
    return i2c1_write_read(BME280_I2C_ADDR, &reg, 1, data, len);
}

static bool write_reg(uint8_t reg, uint8_t value) {
    uint8_t buf[2] = {reg, value};
    return i2c1_write(BME280_I2C_ADDR, buf, sizeof(buf));
}
...
static bool read_calibration(void) {
    ...
    cal.dig_T1 = u16(&buf1[0]);
    cal.dig_T2 = s16(&buf1[2]);
    cal.dig_T3 = s16(&buf1[4]);
    cal.dig_P1 = u16(&buf1[6]);
    cal.dig_P2 = s16(&buf1[8]);
    cal.dig_P3 = s16(&buf1[10]);
    cal.dig_P4 = s16(&buf1[12]);
    cal.dig_P5 = s16(&buf1[14]);
    cal.dig_P6 = s16(&buf1[16]);
    cal.dig_P7 = s16(&buf1[18]);
    cal.dig_P8 = s16(&buf1[20]);
    cal.dig_P9 = s16(&buf1[22]);
    ...
    cal.dig_H3 = buf2[2];
    /* dig_H4/dig_H5 : entiers 12 bits imbriqués sur 3 octets (datasheet §4.2.2) */
...
bool bme280_init(void) {
    ...
    if (!write_reg(REG_RESET, 0xB6U)) return false; /* soft reset (datasheet §5.4.2) */

    if (!read_calibration()) return false;

    if (!write_reg(REG_CTRL_HUM, 0x01U)) return false;   /* oversampling x1 humidité */
    if (!write_reg(REG_CTRL_MEAS, 0x27U)) return false;  /* osrs_t=x1, osrs_p=x1, mode=normal */
    if (!write_reg(REG_CONFIG, 0x00U)) return false;     /* pas de filtre IIR, standby min */
...
bool bme280_read(bme280_sample_t *out) {
    ...
    /* Formule de compensation : même structure que
     * sim/hab_sim/bme280_compensate.py (formules §4.2.3 de la datasheet
     * Bosch, version double précision). Non revalidée par comparaison
     * croisée C/Python dans ce dépôt (contrairement à telemetry.c) —
